...
export interface ProjectHelpRequest {
  id: string;
  userId: string;
  userName: string;
  userEmail: string;
  type: 'request-project-kit' | 'help-with-project'; // Updated type
  timestamp: number;
  status: 'pending' | 'resolved';
  projectTitle?: string; // If help is related to a specific project
  projectDescription?: string; // User's description of the project they need/have
  queryText: string;
  responseText?: string; // Admin's reply
  isKitRequest?: boolean; // New field for requesting the whole kit
}
...
export interface FeedbackItem {
  ...
  timestamp: number;
  status: 'new' | 'reviewed';
  appVersion?: string; // Optional: to track feedback per app version
}
