...
4. **VALUATION (STRICT PRICING RULES IN INR)**: 
   ...
   - **General Rule**: For general/normal PCBs, the total 'asIsValue' must be in the range of **₹30 - ₹50 INR**.
   - **Big PCBs/High Density**: For big or highly dense PCBs (e.g., > 65 components), the value MUST be **BELOW ₹80 INR**.
...
export const analyzePCBImage = async (base64Image: string, mimeType: string): Promise<AnalysisResult> => {
  try {
    ...
    // Fixed: Updated model to 'gemini-3.1-flash-lite' for faster analysis task
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          {
            text: "Analyze this PCB. Count specific component types. Check for DAMAGE. Provide a normal conservative salvage valuation (30-50 INR for general, < 80 INR for big PCBs)."
          }
        ]
      },
      config: {
...
export const sendChatMessage = async (history: { role: string, parts: { text: string }[] }[], message: string, userRole: string = 'Hobbyist'): Promise<string> => {
  try {
    ...
    const systemInstruction = `You are "ResQ". Helpful electronics assistant. User role: ${userRole}. Keep answers calm, concise, helpful and friendly. If anyone asks about components, give a brief idea about them and then suggest them to visit to our ResQ store to check the availability of the component.`;

    // Fixed: Updated model to 'gemini-3.5-flash' for speed and stability
...
export const getProjectGuide = async (title: string, components: string[]): Promise<string[]> => {
  try {
     ...
     const ai = new GoogleGenAI({ apiKey });
     const prompt = `For the electronics project "${title}", using these components: ${components.join(', ')}, provide a highly detailed, clear, and actionable step-by-step build guide.
     Each step should be concise, easy to follow, and logically sequenced. Focus on practical instructions for assembly, wiring, and basic testing.
     Avoid overly long paragraphs in individual steps. Break down complex actions into multiple simpler steps if necessary.`;
