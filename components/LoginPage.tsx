...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const fetchUserProfile = async (firebaseUser: FirebaseUser) => {
    ...
    if (userSnap.exists()) {
        ...
        // Final sanity check: Block admins trying to login via user UI even if they somehow bypass handleLogin
        if (view === 'login' && (profile.role === 'Admin' || firebaseUser.email === ADMIN_EMAIL)) {
            setError("Restricted Access: Administrators must use the Admin Portal.");
            await auth.signOut();
            return;
        }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // SECURITY: Block Main Admin from standard user login
    ...
    try {
        ...
        // Fallback for custom logic / firestore-only passwords
             ...
             try {
                 ...
                 if (userSnap.exists()) {
                     ...
                     if (profile.password === formData.password) {
                         ...
                         // Check Admin-specific demotion or access rules
                         if (view === 'admin-login') {
                            if (profile.role !== 'Admin' && email !== ADMIN_EMAIL) {
                                setError("You are no longer an admin. Contact the admin for that.");
                                setIsLoading(false);
                                return;
                            }
                         ...
                         // Standard Login: Prevent Admin from using it
                         if (view === 'login') {
                             if (profile.role === 'Admin' || email === ADMIN_EMAIL) {
                                 setError("Restricted Access: Administrators must use the Admin Portal.");
                                 setIsLoading(false);
  ...
  const handleSuccessfulAuth = async (user: FirebaseUser) => {
    ...
    if (view === 'admin-login') {
        ...
        if (profile) {
            isAdminMember = profile.role === 'Admin' || user.email === ADMIN_EMAIL;
    ...
    // Standard User Login: Check if user is actually an admin
    if (view === 'login') {
        ...
        if (profile && profile.verified === true) isVerified = true;
  ...
  const handleAccessKeyLogin = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (userSnap.exists()) {
              const profile = userSnap.data() as UserProfile;
              // Strict double-check for demotion
              if (profile.role !== 'Admin' && tempAdminUser.email !== ADMIN_EMAIL) {
                  setError("You are no longer an admin. Contact the admin for that.");
                  setIsLoading(false);
                  return;
              }

              if (profile.loginKey && profile.loginKey === formData.loginKey) {
  ...
  const handleAdminOtpVerify = async (e: React.FormEvent) => {
    ...
    if (otpInput === generatedOtp && tempAdminUser) {
        ...
        if (userSnap.exists()) {
            ...
            // Strict double-check for demotion
            if (data.role !== 'Admin' && tempAdminUser.email !== ADMIN_EMAIL) {
                setError("You are no longer an admin. Contact the admin for that.");
                setIsLoading(false);
                return;
            }
